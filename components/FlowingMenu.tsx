...
interface FlowingMenuProps {
  ...
  borderColor?: string;
  onItemClick?: (link: string, index: number) => void;
}
      ...
      <nav className="menu opacity-30 hover:opacity-100 transition-opacity duration-300">
        {items.map((item, idx) => (
          <MenuItem
            ...
            onItemClick={onItemClick ? () => onItemClick(item.link, idx) : undefined}
          />
        ))}
      </nav>
    </div>
...
function MenuItem({ link, text, image, description, speed, textColor, marqueeBgColor, marqueeTextColor, borderColor, onItemClick }: MenuItemInternalProps) {
